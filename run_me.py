...
for h in qh:
    print(f"     -> {h}")
print(f"   Verdict: {'PASS' if len(qh)>=2 else 'FAIL'}")
...
for x in sorted(u):
    print(f"     - {x}")
print(f"   Verdict: {'PASS' if len(u)>=3 else 'FAIL'}")
...
print(f"B. Semantic Entities: {'PASS' if all_e else 'FAIL'}  (all key entities present)")
print(f"C. Pillar-Cluster:    {'PASS' if pillar else 'FAIL'}  (links to pillar page)")
print(f"D. AEO/GEO:           {'PASS' if len(qh)>=2 else 'FAIL'}  ({len(qh)} question headings)")
print(f"E. Internal Linking:  {'PASS' if len(u)>=3 else 'FAIL'}  ({len(u)} internal links)")
print(f"F. Schema Fields:     {'PASS' if all_f else 'FAIL'}  (all required fields)")
print()
