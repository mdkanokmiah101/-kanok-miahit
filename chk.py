...
cnt = pc.count(pk)
print(f"\nA. TF-IDF COVERAGE")
print(f"   Keyword: '{pk}' -> {cnt} occurrences")
print(f"   {'PASS' if cnt>=5 else 'FAIL'}")

ents = [("Dhaka",["ঢাকা","Dhaka"]),("Bangladesh",["বাংলাদেশ","Bangladesh"]),("Technical SEO",["টেকনিক্যাল SEO"]),("Core Web Vitals",["কোর ওয়েব ভাইটালস","Core Web Vitals"]),("Page Speed",["পেজ স্পিড","Page Speed","সাইট স্পিড"]),("Web/Search",["ওয়েবসাইট","সার্চ ইঞ্জিন"])]
all_e = True
...
print(f"   Link to pillar page: {'PASS' if pillar else 'FAIL'}")
...
for h in qh: print(f"     -> {h}")
print(f"   {'PASS' if len(qh)>=2 else 'FAIL'}")
...
for pfx in ['/blog/','/services/','/industries/','/locations/']:
    for m in re.findall(r'\('+pfx+r'[^)]+\)', pc):
        u.add(m[1:-1])
print(f"   Internal links: {len(u)}")
for x in sorted(u): print(f"     - {x}")
print(f"   {'PASS' if len(u)>=3 else 'FAIL'}")
