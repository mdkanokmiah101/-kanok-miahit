#!/usr/bin/env python3
"""Extract and audit blog posts from data.js - v2 with fixed keyword extraction."""
import re
import sys
...
# Find all post objects - each starts with '{' and slug: 
posts_raw = re.split(r'(?=\n  \{\n\s+slug:)', content)
posts_raw = [p for p in posts_raw if p.strip().startswith('{')]
...
# Manual keyword mapping for better accuracy
KEYWORD_OVERRIDES = {
...
for slug in target_slugs:
    ...
    content_raw = extract_content(post_text)
    
    # Also extract content for the post if it matches a specific pattern
    # Try to get content from the raw text
    content = content_raw
    ...
    # A. TF-IDF Coverage - use manual override for better keywords
    keyword = KEYWORD_OVERRIDES.get(slug, "")
    # Fall back to first part of title before colon
    if not keyword and title:
        if ':' in title:
            keyword = title.split(':')[0].strip()
        else:
            keyword = title.split()[0] if title.split() else title
    ...
    fixes = []
    if kw_count < 5:
        fixes.append(f"- 🔴 **TF-IDF Thin**: Keyword \"{keyword}\" appears only {kw_count} times (need ≥5). Add more natural instances of this keyword throughout the content.")
    if missing_entities:
        fixes.append(f"- 🔴 **Missing Entities**: Add the following entities: {', '.join(missing_entities)}. Include them naturally in paragraphs or headings.")
    if not pillar_link:
        fixes.append(f"- 🔴 **Missing Pillar Link**: No link to the \"{pillar}\" pillar page found. Add a contextual link (e.g., `/blog/...` or `/services/...`).")
    if q_headings < 2:
