#!/usr/bin/env python3
"""Content Framework Enforcer v4 — final keyword extraction fix."""
...
def extract_keywords(title):
    """Extract best primary keyword candidates from the title."""
    if not title:
        return ["seo"]
    t = title.strip()
    candidates = []
    
    # Pattern: "X SEO Case Study:..." or "X SEO:..." → brand X
    m = re.match(r'^(.+?)\s+(SEO|Case Study)\s*[:\-–—]', t, re.IGNORECASE)
    if m:
        brand = m.group(1).strip()
        candidates.append(brand)
        candidates.append(f"{brand} SEO")
        return candidates
    
    # Pattern: "X SEO Case Study:..." (no space before SEO)
    m = re.match(r'^(.+?)\s+SEO\s+Case\s+Study', t, re.IGNORECASE)
    if m:
        brand = m.group(1).strip()
        candidates.append(brand)
        candidates.append(f"{brand} SEO")
    ...
    # Pattern: "SEO Case Study: How..." → keyword is "SEO" or case-study topic
    m = re.match(r'^SEO\s+Case\s+Study\s*:\s*(.+)$', t, re.IGNORECASE)
    if m:
        rest = m.group(1).strip()
        # Take the first meaningful NP
        words = re.sub(r'^how\s+|^what\s+|^why\s+', '', rest, flags=re.IGNORECASE).split()
        np = [w for w in words if w.lower() not in {'a','an','the','in','of','to','for','and','or','from','by','with','at'}]
        if np:
            candidates.append(' '.join(np[:3]))
        candidates.append('SEO')
        return candidates
    
    # Pattern: "X vs Y" → X and Y as keywords
    m = re.match(r'^(.+?)\s+vs\.?\s+(.+)$', t, re.IGNORECASE)
    if m:
        a, b = m.group(1).strip(), m.group(2).strip()
        # Take first 3 words of each
        a_kw = ' '.join(a.split()[:3]).rstrip(':,-—')
        b_kw = ' '.join(b.split()[:3]).rstrip(':,-—')
        candidates.extend([a_kw, b_kw])
        return candidates
    
    # Pattern: "Top N X (And How to Fix Them)" → X
    m = re.match(r'^(Top\s+\d+\s+)(.+?)(?:\s*\(.*)?$', t, re.IGNORECASE)
    if m:
        candidates.append(m.group(2).strip())
        return candidates
    
    # Pattern: "Why/How/What X..." → skip question word
    m = re.match(r'^(How|What|Why|When|Where|Can|Do|Is|Are|Does|Which)\s+(.+)$', t, re.IGNORECASE)
    if m:
        rest = m.group(2).strip()
        stopwords = {'a','an','the','for','in','of','to','and','is','are','was','were','your','you','it','its','with','from','by','at','on','do','does','did','can','will','would','could','should'}
        words = rest.split()
        kw = []
        for w in words:
            cw = w.strip('.,:;!?"\'()-')
            if cw.lower() in stopwords and not kw: continue
            kw.append(cw)
            if len(kw) >= 4: break
        if kw:
    ...
    stopwords = {'a','an','the','for','in','of','to','and','is','are','was','were'}
    words = t.split()
    kw = [w.strip('.,:;!?"\'()-') for w in words if w.lower() not in stopwords]
    if kw:
        candidates.append(' '.join(kw[:3]))
    else:
        candidates.append(t[:30])
    return candidates
...
def check_tfidf(post):
    ...
    # Check if first word of brand appears enough (for brands mentioned without full name)
    if best_count < 5 and keywords:
    ...
    # For posts < 4000 chars, lower threshold
    threshold = 3 if len(content) < 4000 else 5
...
def check_entities(post):
    ...
    for loc, name in [('dhaka', 'Dhaka'), ('bangladesh', 'Bangladesh')]:
        if loc not in content_lower: entities_needed.append(name)
    if not any(s.lower() in content_lower for s in ['seo', 'search engine optimization']):
...
def check_internal_links(post):
    ...
    all_internal = set()
    for text, url in links: all_internal.add(url)
    for url in bare: all_internal.add(url.rstrip('.,;:!?)'))
...
def check_schema(post):
    issues = []
    if not post.get('title'): issues.append('title missing')
    if not post.get('excerpt'): issues.append('excerpt missing')
    if not post.get('date'): issues.append('date missing')
...
for slug in MODIFIED_SLUGS:
    ...
    if not tfidf_pass: all_passed = False; post_passed = False
    ...
    if not entities_pass: all_passed = False; post_passed = False
    ...
    if not pillar_pass: all_passed = False; post_passed = False
    
    aeo_pass, q_count = check_aeo_geo(post)
    aeo_status = '✅' if aeo_pass else '❌'
    if not aeo_pass: all_passed = False; post_passed = False
    
    link_pass, link_count, link_urls = check_internal_links(post)
    link_status = '✅' if link_pass else '❌'
    if not link_pass: all_passed = False; post_passed = False
    ...
    if not schema_pass: all_passed = False; post_passed = False
    
    if post_passed: passed_count += 1
...
""")
    ...
    if not aeo_pass:
        fix_parts.append(f"- ❓ **AEO/GEO:** Add {2 - q_count} more question-based H2/H3 headings (starting with How/What/Why/Can/Do/Is/Are) to capture AI Overview and voice search queries. Use FAQ sections with question headers.")
    if not link_pass:
