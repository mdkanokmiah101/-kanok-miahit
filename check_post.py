...
print(f"   Occurrences in content: {cnt}")
...
print(f"   Tags: {tags}")
print(f"   Pillar: টেকনিক্যাল SEO (Technical SEO)")
pillar = "/services/technical-seo" in pc
pillar2 = "/blog/technical-seo-checklist-bangladeshi-websites" in pc
print(f"   Link to /services/technical-seo: {'✅' if pillar else '❌'}")
print(f"   Link to technical SEO blog: {'✅' if pillar2 else '❌'}")
print(f"   Verdict: {'✅ PASS' if pillar else '❌ FAIL (no pillar link)'}")
...
for raw in pc.split('\n'):
    ...
    if is_q: qh.append(l)
print(f"   Question-based headings: {len(qh)}")
for h in qh:
    print(f"     → {h}")
...
for fn,pat in flds.items():
    ...
    print(f"   {fn}: {'✅' if p else '❌'}")
    if not p: all_f = False
print(f"   Verdict: {'✅ PASS' if all_f else '❌ FAIL'}")

print("\n" + "="*70)
print("SUMMARY")
print("="*70)
print(f"A. TF-IDF Coverage:   {'✅' if cnt>=5 else '❌'}  {cnt} occurrences of '{pk}'")
print(f"B. Semantic Entities: {'✅' if all_e else '❌'}  (all key entities present)")
print(f"C. Pillar-Cluster:    {'✅' if pillar else '❌'}  (links to pillar page)")
print(f"D. AEO/GEO:           {'✅' if len(qh)>=2 else '❌'}  ({len(qh)} question headings)")
print(f"E. Internal Linking:  {'✅' if len(u)>=3 else '❌'}  ({len(u)} internal links)")
print(f"F. Schema Fields:     {'✅' if all_f else '❌'}  (all required fields)")
print()
