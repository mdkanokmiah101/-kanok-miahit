#!/usr/bin/env python3
"""Run all 6 framework checks on 'ai-seo-2026-dhaka-experts-optimize-google-ai-chatgpt'."""
import re, sys
...
# ============================================================
# Primary keywords for this post: "ai seo", "google ai overview", chatgpt, geo
kw = "ai seo"
alt_kw = ["ai seo", "google ai overview", "chatgpt", "geo optimization", "entity seo", 
          "generative engine optimization", "ai search", "seo expert", "dhaka",
          "google ai overviews", "eeat", "e-e-a-t", "answer engine optimization"]
...
if 'ai' not in c_lower: missing_ents.append("AI")
if 'seo' not in c_lower: missing_ents.append("SEO")
if 'chatgpt' not in c_lower: missing_ents.append("ChatGPT")
ent_pass = len(missing_ents) == 0
print(f"\nCRITERION 2: Entities")
print(f"  Required: Dhaka, Bangladesh, Google, AI, SEO, ChatGPT")
...
for h in headings:
    ...
    for q in qw:
        if s.startswith(q):
            qh.append(h.strip())
            break
    # Also catch headings with '?' in them (FAQ section)
...
print()
print('=' * 60)
print('| {:<24} | {:<8} | {:<40} |'.format('Check', 'Status', 'Details'))
print('|{:-<24}-+-{:-<8}-+-{:-<40}-|'.format('', '', ''))
print('| {:<24} | {:<8} | {:<40} |'.format('TF-IDF Keyword Density', tfidf_status, tfidf_detail))
print('| {:<24} | {:<8} | {:<40} |'.format('', '', tfidf_count))
print('| {:<24} | {:<8} | {:<40} |'.format('Entities', ent_status, ent_detail))
print('| {:<24} | {:<8} | {:<40} |'.format('Pillar Link', pil_status, pil_detail))
print('| {:<24} | {:<8} | {:<40} |'.format('AEO/GEO Questions', aeo_status, aeo_detail))
print('| {:<24} | {:<8} | {:<40} |'.format('Internal Links', il_status, il_detail))
print('| {:<24} | {:<8} | {:<40} |'.format('Schema Readiness', sch_status, sch_detail))
print('=' * 60)
